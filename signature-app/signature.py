...
def generate_signature(document_path, key_path, aes_key):
    ...
    private_key = RSA.import_key(decrypt_private_key(aes_key, key_path))
    signature = pkcs1_15.new(private_key).sign(document_hash)
    return signature


def create_xml_document(document_path, user_info, key_path, aes_key):
    document_info = {
    ...
    ET.SubElement(root, "Signature").text = generate_signature(document_path, key_path, aes_key).hex()
